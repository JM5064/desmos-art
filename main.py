...
def calculate_circle2(coord_set):
    a1, a2, a3 = coord_set[0][0], coord_set[1][0], coord_set[2][0]
    b1, b2, b3 = coord_set[0][1], coord_set[1][1], coord_set[2][1]
...
for p in points:
    ...
    if is_collinear(p):
        if is_vertical(p):
            print("x = %d \\left\\{%d < y < %d\\right\\}"
                  % (p[0][0], y_bounds[0], y_bounds[1]))
        else:
            slope = calculate_slope(p)
            print("y - %d = %d(x - %d) \\left\\{%d < x < %d\\right\\}" % (p[0][1], slope, p[0][0], x_bounds[0], x_bounds[1]))
    else:
        circle = calculate_circle2(p)
        if circle is not None:
            print("(x - %d)^2 + (y - %d)^2 = %d \\left\\{%d < x < %d\\right\\} \\left\\{%d < y < %d\\right\\}"
